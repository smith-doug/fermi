...
/**
 * @brief     Class for setting up the MoveIt environment.
 * @details   The GenerateCartesianPath Class handles all the interactions with the MoveIt environment.
 *            This Class inherits from the QObject superclass.
 *            The concept of this class is to initialize all the necessary parameters for generating Cartesian Path from
 *            Way-Points. It gets the necessary Way-Points and handles them in the Cartesian Path Planner of the MoveIt.
 *            Furthermore it checks if a Way-Point is inside the IK solution for the loaded robot and changes it state
 *            correspondingly.
 * @author    Risto Kojcev
 **/
...
class GenerateCartesianPath: public QObject
{
	...
	Q_SIGNALS:
		...
		void updateCurrentPosition_signal(const std::string robot_model_frame, const tf::Transform end_effector);
	...
	protected:
		...
		int selected_plan_group_;
		std::string target_frame_;
		std::string end_effector_frame_;
		std::vector< const moveit::core::JointModelGroup * >  end_eff_joint_groups_;

		/**
		 * MoveIt and Cartesian path parameters set by the user from the QT UI Parameter for setting the planning time
		 * of the MoveIt.
		 */
		double PLAN_TIME_;
